              ...
              {data.changes.map((item, index) => (
                <SidebarMenuItem key={index}>
                  <SidebarMenuButton>
                    <File />
                    <Link
                      href={`/veryBeginnerTutorials/MainTopics/${item.file.replaceAll(" ", "")}`}
                      className="w-full"
                    >
                      {item.file}
                    </Link>
                  ...
                  <div key={index} className="w-full">
                    <div
                      onClick={(e) => {
                        ...
                        console.log(
                          "Navigating to:",
                          `/veryBeginnerTutorials/Rubriques/${trimmedHeader}`,
                        );
...
function Tree({ item, indexes }: { item: string | any[]; indexes: string[] }) {
  ...
  const incrementIndexes = indexes.find(
    (i: string) => i === name.replaceAll(" ", ""),
  )
    ? [...indexes]
    : [...indexes, name.replaceAll(" ", "")];
        ...
        <CollapsibleContent>
          <SidebarMenuSub>
            {items.map((subItem, index) => {
              ...
              const addSubIndexes = incrementIndexes.includes(itemKey)
                ? incrementIndexes
                : [...incrementIndexes, itemKey];
              return (
